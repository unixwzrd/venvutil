#!/bin/bash
#
# macOS-webui COnfiguration File
#
#
...
#   "pip        | Stangard pip install for Torch                                    | pytorch   | torch |                                                      | pip   | torchvision torchaudio"
    "base       | Base Conda install for Torch                                      | pytorch   | |                                                            | conda | torchvision torchaudio -c pytorch --force-reinstall -y"
#   "llama      | Stangard pip install for lama-cpp-python                          | llama-cpp-python==0.1.78 | | CMAKE_ARGS='-DLLAMA_METAL=on' FORCE_CMAKE=1 | pip   | --force-reinstall --no-deps"
    "llama      | Stangard pip install for lama-cpp-python                          | llama-cpp-python | | NPY_BLAS_ORDER='accelerate' NPY_LAPACK_ORDER='accelerate' CMAKE_ARGS='-DLLAMA_METAL=on' FORCE_CMAKE=1 | pip   | "
